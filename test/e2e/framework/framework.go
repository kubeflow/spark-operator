// Copyright 2016 The prometheus-operator & Kubernetes Authors
//
...
import (
	...
	"k8s.io/spark-on-k8s-operator/test/e2e/testutil"
...
func New(ns, kubeconfig, opImage string) (*Framework, error) {
	...
	namespace, err := testutil.CreateNamespace(cli, ns)
	if err != nil {
...
func (f *Framework) setupOperator(opImage string) error {
	if _, err := testutil.CreateServiceAccount(f.KubeClient, f.Namespace.Name, "../../manifest/spark-operator-rbac.yaml"); err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to create operator service account")
	}

	if err := testutil.CreateClusterRole(f.KubeClient, "../../manifest/spark-operator-rbac.yaml"); err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to create cluster role")
	}

	if _, err := testutil.CreateClusterRoleBinding(f.KubeClient, f.Namespace.Name, "../../manifest/spark-operator-rbac.yaml"); err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to create cluster role binding")
	}

	deploy, err := testutil.MakeDeployment("../../manifest/spark-operator.yaml")
	if err != nil {
		return err
	}
	...
	err = testutil.CreateDeployment(f.KubeClient, f.Namespace.Name, deploy)
	if err != nil {
		return err
	}

	opts := metav1.ListOptions{LabelSelector: fields.SelectorFromSet(fields.Set(deploy.Spec.Template.ObjectMeta.Labels)).String()}
	err = testutil.WaitForPodsReady(f.KubeClient, f.Namespace.Name, f.DefaultTimeout, 1, opts)
	if err != nil {
...
func (f *Framework) Teardown() error {
	if err := testutil.DeleteClusterRole(f.KubeClient, "../../manifest/spark-operator-rbac.yaml"); err != nil && !apierrors.IsAlreadyExists(err) {
		return errors.Wrap(err, "failed to delete operator cluster role")
	}

	if err := testutil.DeleteClusterRoleBinding(f.KubeClient, "../../manifest/spark-operator-rbac.yaml"); err != nil && !apierrors.IsAlreadyExists(err) {
	...
	if err := testutil.DeleteNamespace(f.KubeClient, f.Namespace.Name); err != nil {
		return err
	}
