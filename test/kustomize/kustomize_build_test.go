...
func TestKustomizeBuild(t *testing.T) {
	...
	t.Run("ControllerRBAC", func(t *testing.T) {
		...
		roleObj := findResource(resources, "Role", "controller")
	...
	t.Run("WebhookClusterRole", func(t *testing.T) {
		...
		for _, res := range []string{"resourcequotas", "sparkapplications", "mutatingwebhookconfigurations"} {
			assert.True(t, rulesHaveResource(cr.Rules, res),
				"webhook ClusterRole should have '%s'", res)
		}

		for _, res := range []string{"events", "pods", "scheduledsparkapplications"} {
			assert.False(t, rulesHaveResource(cr.Rules, res),
				"webhook ClusterRole should NOT have '%s' (least privilege)", res)
		}
	...
	t.Run("WebhookRole", func(t *testing.T) {
		obj := findResource(resources, "Role", "webhook")
		...
		assert.False(t, rulesHaveResource(role.Rules, "events"),
			"webhook Role should NOT have 'events'")
	})
	...
	t.Run("WebhookConfiguration", func(t *testing.T) {
		...
		assert.GreaterOrEqual(t, svcRefCount, 4,
			"webhook configs should reference 'webhook-service' (got %d)", svcRefCount)
	})
	...
	t.Run("DeploymentConfiguration", func(t *testing.T) {
		...
		assert.Contains(t, saNames, "controller", "expected serviceAccountName 'controller'")
		assert.Contains(t, saNames, "webhook", "expected serviceAccountName 'webhook'")
