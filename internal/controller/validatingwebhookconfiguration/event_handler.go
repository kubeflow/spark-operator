...
func (h *EventHandler) Create(ctx context.Context, event event.CreateEvent, queue workqueue.RateLimitingInterface) {
	...
	logger.V(1).Info("ValidatingWebhookConfiguration created", "name", vwc.Name, "namespace", vwc.Namespace)
...
func (h *EventHandler) Update(ctx context.Context, event event.UpdateEvent, queue workqueue.RateLimitingInterface) {
	...
	logger.V(1).Info("ValidatingWebhookConfiguration updated", "name", newWebhook.Name, "namespace", newWebhook.Namespace)
	key := types.NamespacedName{
...
func (h *EventHandler) Delete(ctx context.Context, event event.DeleteEvent, queue workqueue.RateLimitingInterface) {
	...
	logger.V(1).Info("ValidatingWebhookConfiguration deleted", "name", vwc.Name, "namespace", vwc.Namespace)
	key := types.NamespacedName{
		Namespace: vwc.Namespace,
		Name:      vwc.Name,
	}
...
func (h *EventHandler) Generic(ctx context.Context, event event.GenericEvent, queue workqueue.RateLimitingInterface) {
	...
	logger.V(1).Info("ValidatingWebhookConfiguration generic event", "name", vwc.Name, "namespace", vwc.Namespace)
	key := types.NamespacedName{
		Namespace: vwc.Namespace,
		Name:      vwc.Name,
	}
