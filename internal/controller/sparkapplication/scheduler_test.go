...
func TestShouldDoBatchScheduling(t *testing.T) {
	...
	t.Run("workload scheduler selected when registered", func(t *testing.T) {
		...
		err := testRegistry.Register(
			workload.SchedulerName,
			func(config scheduler.Config) (scheduler.Interface, error) {
...
func TestSubmitSparkApplicationUsesEphemeralSchedulingCopy(t *testing.T) {
	...
	mock := &mockScheduler{
		...
		scheduleFunc: func(app *v1beta2.SparkApplication) error {
			...
			app.Spec.Executor.Template.Spec.SchedulingGroup = &corev1.PodSchedulingGroup{
				PodGroupName: ptr.To("test-pod-group"),
			}
			return nil
		},
	}
	...
	require.NoError(
		...
		testRegistry.Register(
			workload.SchedulerName,
			func(scheduler.Config) (scheduler.Interface, error) {
				return mock, nil
			},
		),
	)
	...
	if app.Spec.Executor.Template != nil {
		assert.Nil(
			t,
			app.Spec.Executor.Template.Spec.SchedulingGroup,
			"operator-managed schedulingGroup must not be persisted to the reconciled object",
		)
	}

	require.NotNil(t, submitter.app.Spec.Executor.Template)
	require.NotNil(t, submitter.app.Spec.Executor.Template.Spec.SchedulingGroup)
	assert.Equal(
		t,
		"test-pod-group",
		*submitter.app.Spec.Executor.Template.Spec.SchedulingGroup.PodGroupName,
	)
...
func (s *capturingSubmitter) Submit(
	_ context.Context,
	app *v1beta2.SparkApplication,
) error {
	s.app = app
