...
func nameOption(app *v1beta2.SparkApplication) ([]string, error) {
	args := []string{
		"--conf",
		fmt.Sprintf("%s=%s", common.SparkAppName, app.Name),
	}
	return args, nil
}
...
func driverVolumeMountsOption(app *v1beta2.SparkApplication) ([]string, error) {
	...
	for _, volumeMount := range volumeMounts {
		...
		if !ok {
			return args, fmt.Errorf("volume %s not found", volumeMount.Name)
		}
		switch {
...
func driverEnvOption(app *v1beta2.SparkApplication) ([]string, error) {
	...
	for key, value := range app.Spec.Driver.EnvVars {
		...
		args = append(args, fmt.Sprintf("%s=%s", property, value))
	}
	return args, nil
}
...
func executorConfOption(app *v1beta2.SparkApplication) ([]string, error) {
	...
	for key, value := range app.Spec.Executor.Annotations {
		args = append(args,
			fmt.Sprintf("%s%s=%s", common.SparkKubernetesExecutorAnnotationPrefix, key, value))
	}
	...
	for key, value := range app.Spec.Executor.EnvSecretKeyRefs {
		...
		args = append(args, fmt.Sprintf("%s=%s:%s", property, value.Name, value.Key))
	}

	if app.Spec.Executor.JavaOptions != nil {
		args = append(args,
			fmt.Sprintf("%s=%s", common.SparkExecutorExtraJavaOptions, *app.Spec.Executor.JavaOptions))
	}
