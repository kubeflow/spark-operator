...
func TestApplyDefaultDriverServiceAccount(t *testing.T) {
	newApp := func(serviceAccount *string) *v1beta2.SparkApplication {
	...
	t.Run("returns the application unchanged when no default is configured", func(t *testing.T) {
		app := newApp(nil)
		got := applyDefaultDriverServiceAccount(app, "")
		assert.Same(t, app, got)
		assert.Nil(t, got.Spec.Driver.ServiceAccount)
	})

	t.Run("applies the default when the application does not specify one", func(t *testing.T) {
		app := newApp(nil)
		got := applyDefaultDriverServiceAccount(app, "spark-operator-spark")
		assert.NotSame(t, app, got)
		assert.Equal(t, "spark-operator-spark", *got.Spec.Driver.ServiceAccount)
		assert.Nil(t, app.Spec.Driver.ServiceAccount)
	})

	t.Run("applies the default when the application specifies an empty service account", func(t *testing.T) {
		app := newApp(ptr.To(""))
		got := applyDefaultDriverServiceAccount(app, "spark-operator-spark")
		assert.NotSame(t, app, got)
		assert.Equal(t, "spark-operator-spark", *got.Spec.Driver.ServiceAccount)
		assert.Equal(t, "", *app.Spec.Driver.ServiceAccount)
	})

	t.Run("prefers the service account specified by the application", func(t *testing.T) {
		app := newApp(ptr.To("custom-sa"))
		got := applyDefaultDriverServiceAccount(app, "spark-operator-spark")
		assert.Same(t, app, got)
		assert.Equal(t, "custom-sa", *got.Spec.Driver.ServiceAccount)
	})

	t.Run("propagates the default into the driver configuration", func(t *testing.T) {
		app := newApp(nil)
		got := applyDefaultDriverServiceAccount(app, "spark-operator-spark")

		args, err := driverConfOption(got)
		assert.NoError(t, err)
		assert.Contains(t, args, fmt.Sprintf("%s=%s", common.SparkKubernetesAuthenticateDriverServiceAccountName, "spark-operator-spark"))

		args, err = driverConfOption(app)
		assert.NoError(t, err)
		assert.NotContains(t, args, fmt.Sprintf("%s=%s", common.SparkKubernetesAuthenticateDriverServiceAccountName, "spark-operator-spark"))
	})
}
