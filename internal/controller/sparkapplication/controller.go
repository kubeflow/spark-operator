...
func (r *Reconciler) submitSparkApplication(ctx context.Context, app *v1beta2.SparkApplication) {
	...
	// Apply the default service account to the ephemeral submission copy.
	submitApp := util.ApplyDefaultDriverServiceAccount(
		submissionApp,
		r.options.DefaultServiceAccount,
	)
	if submitApp != submissionApp {
		logger.Info(
			"Applied default driver service account",
			"serviceAccount", r.options.DefaultServiceAccount,
		)
	}
	submissionApp = submitApp
