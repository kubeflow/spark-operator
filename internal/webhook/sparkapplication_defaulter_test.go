...
func TestDefaultedSparkApplicationIsValid_LotsOfNestedOmitedValues(t *testing.T) {
	jsonData := []byte(`{
		...
		"spec": {
		  ...
		  "monitoring": {
			...
			"metricsPropertiesFile": "dummy"
		  },
		  "batchSchedulerOptions": {},
		  "driverIngressOptions": [{"servicePort": 4040, "servicePortName": "test"}],
		  "dynamicAllocation": {}
		}
	  }`)
...
func TestDefaultedSparkApplicationIsValid_LotsOfHighLevelOmitedValues(t *testing.T) {
	jsonData := []byte(`{
		"apiVersion": "sparkoperator.k8s.io/v1beta2",
		"kind": "SparkApplication",
		"metadata": {
