...
func launchedBySparkOperator(meta metav1.ObjectMeta) bool {
	val, present := meta.Labels[common.LabelLaunchedBySparkOperator]
...
func resourceUsage(spec v1beta2.SparkApplicationSpec) (ResourceList, error) {
...
func sparkApplicationResourceUsage(sparkApp v1beta2.SparkApplication) (ResourceList, error) {
	// A completed/failed SparkApplication consumes no resources
	if !sparkApp.Status.TerminationTime.IsZero() || sparkApp.Status.AppState.State == v1beta2.ApplicationStateFailed || sparkApp.Status.AppState.State == v1beta2.ApplicationStateCompleted {
		return ResourceList{}, nil
	}
	return resourceUsage(sparkApp.Spec)
}

func scheduledSparkApplicationResourceUsage(sparkApp v1beta2.ScheduledSparkApplication) (ResourceList, error) {
	// Failed validation, will consume no resources
	if sparkApp.Status.ScheduleState == v1beta2.ScheduleStateFailedValidation {
		return ResourceList{}, nil
	}
