...
func TestSchedule_ClusterMode(t *testing.T) {
	...
	if app.Spec.Driver.Template != nil &&
		app.Spec.Driver.Template.Spec.SchedulingGroup != nil {
		t.Error("Driver SchedulingGroup should not be set (driver schedules independently)")
	}

	if app.Spec.Executor.Template == nil ||
		app.Spec.Executor.Template.Spec.SchedulingGroup == nil {
		t.Error("Executor SchedulingGroup not set")
	} else if *app.Spec.Executor.Template.Spec.SchedulingGroup.PodGroupName != "test-app-test-submission-1" {
		t.Errorf("Executor PodGroupName = %s, want test-app-test-submission-1", *app.Spec.Executor.Template.Spec.SchedulingGroup.PodGroupName)
	}
}
