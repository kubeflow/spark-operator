...
func (s *Scheduler) Schedule(app *v1beta2.SparkApplication) error {
	...
	if app.Spec.Executor.Template == nil {
		app.Spec.Executor.Template = &corev1.PodTemplateSpec{}
	}

	app.Spec.Executor.Template.Spec.SchedulingGroup = &corev1.PodSchedulingGroup{
		PodGroupName: ptr.To(podGroup.Name),
	}

	logger.Info(
		"Stamped executor pod template with schedulingGroup",
		"app", app.Name,
...
func (s *Scheduler) Cleanup(app *v1beta2.SparkApplication) error {
	...
	if !metav1.IsControlledBy(pg, app) {
		return fmt.Errorf(
			"refusing to delete PodGroup %s/%s because it is not controlled by SparkApplication %s (UID %s)",
			namespace,
			podGroupName,
			app.Name,
			app.UID,
...
// syncWorkload ensures the requested Workload exists and is immutable.
func (s *Scheduler) syncWorkload(
	...
	if err == nil {
		if !metav1.IsControlledBy(existing, app) {
			return fmt.Errorf(
				"workload %s/%s is not controlled by SparkApplication %s (UID %s)",
				desired.Namespace,
				desired.Name,
				app.Name,
				app.UID,
		...
		if !apiequality.Semantic.DeepDerivative(desired.Spec, existing.Spec) {
			return fmt.Errorf(
				"workload %s/%s has an immutable spec that differs from the requested scheduling policy",
				desired.Namespace,
				desired.Name,
			)
		}
...
// syncPodGroup ensures the requested PodGroup exists and is immutable.
func (s *Scheduler) syncPodGroup(
	...
	if err == nil {
		if !metav1.IsControlledBy(existing, app) {
			return fmt.Errorf(
				"pod group %s/%s is not controlled by SparkApplication %s (UID %s)",
				desired.Namespace,
				desired.Name,
				app.Name,
				app.UID,
		...
		if !apiequality.Semantic.DeepDerivative(desired.Spec, existing.Spec) {
			return fmt.Errorf(
				"pod group %s/%s has an immutable spec that differs from the requested scheduling policy",
				desired.Namespace,
				desired.Name,
			)
		}
...
// buildWorkload constructs the Workload policy template for the given SparkApplication.
func buildWorkload(
	app *v1beta2.SparkApplication,
	minCount int32,
	...
	if app.Spec.BatchSchedulerOptions != nil &&
		app.Spec.BatchSchedulerOptions.PriorityClassName != nil {
		podGroupTemplate.PriorityClassName =
			*app.Spec.BatchSchedulerOptions.PriorityClassName
	}
...
// buildPodGroup constructs the per-submission PodGroup for the given SparkApplication.
func buildPodGroup(
	app *v1beta2.SparkApplication,
	workloadName string,
	minCount int32,
