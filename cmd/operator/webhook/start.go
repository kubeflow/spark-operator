...
import (
	...
	"github.com/kubeflow/spark-operator/pkg/cert"
...
func NewStartCommand() *cobra.Command {
	...
	command.Flags().BoolVar(&enableWebhook, "enable-webhook", false, "Whether to enable the webhooks for mutating and validating Spark pods.")
	...
	command.Flags().BoolVar(&enableMetrics, "enable-metrics", false, "Whether to enable the metrics endpoint.")
	command.Flags().StringVar(&metricsPort, "metrics-port", "10254", "Port for the metrics endpoint.")
...
func start() {
	...
	certProvider := cert.NewCertProvider(
		client,
		webhookServiceName,
	...
	if err := wait.ExponentialBackoff(
		...
		func() (bool, error) {
			...
			if err := certProvider.SyncSecret(webhookSecretName, webhookSecretNamespace); err != nil {
	...
	if err := ctrl.NewWebhookManagedBy(mgr).
		...
		WithValidator(webhook.NewSparkApplicationValidator()).
		Complete(); err != nil {
