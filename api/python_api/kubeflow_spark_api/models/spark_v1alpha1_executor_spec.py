...
class SparkV1alpha1ExecutorSpec(BaseModel):
    ...
    """ # noqa: E501
    core_limit: Optional[StrictStr] = Field(default=None, description="CoreLimit specifies the physical CPU limit for the pod, controlling the Kubernetes CPU limit. This is independent of Cores. For a SparkConnect server, the operator applies this directly as the CPU limit on the operator-created server pod. For a SparkConnect executor, this maps to `spark.kubernetes.executor.limit.cores`. Valid values follow Kubernetes quantity format (e.g., \"500m\", \"1\", \"1.5\").", alias="coreLimit")
    core_request: Optional[StrictStr] = Field(default=None, description="CoreRequest specifies the physical CPU request for the pod, controlling the Kubernetes CPU request. This is independent of Cores. For a SparkConnect server, the operator applies this directly as the CPU request on the operator-created server pod. For a SparkConnect executor, this maps to `spark.kubernetes.executor.request.cores`. Valid values follow Kubernetes quantity format (e.g., \"500m\", \"1\", \"1.5\").", alias="coreRequest")
