...
* [Driver UI Access, Service annotations and Ingress](#driver-ui-access-service-annotations-and-ingress)
...
## Driver UI Access, Service annotations and Ingress
...
If you are using API gateway that depends on service annotations to expose services (e.g. [Ambassador](https://www.getambassador.io/)), you can configure a templated service annotations by setting the `spark-ui-service-annotations` command-line flag. The `spark-ui-service-annotations` should be a JSON string and supports the following mustache variables:
- `{{$appName}}`
- `{{$serviceName}}`
- `{{$namespace}}`
