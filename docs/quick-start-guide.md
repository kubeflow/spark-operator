...
* [Configuration](#configuration)
    * [Customizing operator](#customizing-operator)
    * [Mutating Admission Webhook](#mutating-admission-webhook)
    * [Default Spark Image](#default-spark-image)
    * [Metrics](#metrics)
* [Upgrade](#upgrade)
...
The operator uses multiple workers in the `SparkApplication` controller and the submission runner.
...
Operator allows to set default unified spark image for driver and executors, using `-default-spark-image=[image]:[tag]` flag. Each submitted spark application will use that image as default, upon customized by the user in the `SparkApplication` template.
