...
Below is an example that shows how to configure the metric system to expose metrics to Prometheus using the Prometheus JMX exporter. Note that the JMX exporter Java agent jar is listed as a dependency and will be downloaded to where `.spec.dep.jarsDownloadDir` points to in Spark 2.3.x, which is `/var/spark-data/spark-jars` by default. 
...
Things are different in Spark 2.4 and above as dependencies will be downloaded to the local working directory instead in Spark 2.4, which is not known until at runtime. So with Spark 2.4 and above, the recommendation is to put the jar into a custom image as demonstrated in this [Dockerfile](../spark-docker/DockerFile) and specifies the path to the jar in `jmxExporterJar`. A complete example can be found in [examples/spark-pi-prometheus.yaml](../examples/spark-pi-prometheus.yaml).
