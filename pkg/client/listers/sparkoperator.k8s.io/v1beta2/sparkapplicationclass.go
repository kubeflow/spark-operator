...
type SparkApplicationClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.SparkApplicationClass, err error)
	// Get retrieves the SparkApplicationClass from the index for a given name.
	Get(name string) (*v1beta2.SparkApplicationClass, error)
...
// List lists all SparkApplicationClasses in the indexer.
func (s *sparkApplicationClassLister) List(selector labels.Selector) (ret []*v1beta2.SparkApplicationClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.SparkApplicationClass))
	})
	return ret, err
}

// Get retrieves the SparkApplicationClass from the index for a given name.
func (s *sparkApplicationClassLister) Get(name string) (*v1beta2.SparkApplicationClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("sparkapplicationclass"), name)
	}
	return obj.(*v1beta2.SparkApplicationClass), nil
}
