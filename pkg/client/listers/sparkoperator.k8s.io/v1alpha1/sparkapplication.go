...
// List lists all SparkApplications in the indexer.
func (s *sparkApplicationLister) List(selector labels.Selector) (ret []*v1alpha1.SparkApplication, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.SparkApplication))
	})
	return ret, err
}
...
// List lists all SparkApplications in the indexer for a given namespace.
func (s sparkApplicationNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.SparkApplication, err error) {
...
// Get retrieves the SparkApplication from the indexer for a given namespace and name.
func (s sparkApplicationNamespaceLister) Get(name string) (*v1alpha1.SparkApplication, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("sparkapplication"), name)
	}
	return obj.(*v1alpha1.SparkApplication), nil
}
