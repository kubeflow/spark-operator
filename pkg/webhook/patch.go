...
func addOwnerReference(pod *corev1.Pod, app *v1beta2.SparkApplication) {
	...
	for _, o := range pod.OwnerReferences {
		if equality.Semantic.DeepEqual(o, ownerReference) {
			return
		}
	}
...
func addVolume(pod *corev1.Pod, volume corev1.Volume) {
	for _, o := range pod.Spec.Volumes {
		if equality.Semantic.DeepEqual(o, volume) {
			return
		}
	}
...
func addVolumeMount(pod *corev1.Pod, mount corev1.VolumeMount) {
	...
	for _, o := range pod.Spec.Containers[i].VolumeMounts {
		if equality.Semantic.DeepEqual(o, mount) {
			return
		}
	}
