...
// If selectorString is empty, only explicit namespaces are matched.
// If explicitNamespaces contains empty string, all namespaces are matched.
func NewNamespaceMatcher(explicitNamespaces []string, selectorString string) (*NamespaceMatcher, error) {
	nsMap := make(map[string]bool)
	if len(explicitNamespaces) == 0 {
...
func (m *NamespaceMatcher) MatchesWithClient(ctx context.Context, c client.Client, namespaceName string) (bool, error) {
	// If no selector is configured, use fast-path check
	if !m.hasSelector {
		return m.Matches(namespaceName, nil), nil
	}
	...
	return m.MatchesNamespace(ns), nil
}
