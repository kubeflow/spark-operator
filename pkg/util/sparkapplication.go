...
func GetDriverEnvVarConfOptions(app *v1beta2.SparkApplication) []string {
	...
	for key, value := range app.Spec.Driver.EnvVars {
		envVar := fmt.Sprintf("%s%s=%s", common.SparkKubernetesDriverEnvPrefix, key, value)
...
func GetExecutorEnvVarConfOptions(app *v1beta2.SparkApplication) []string {
	...
	for key, value := range app.Spec.Executor.EnvVars {
		envVar := fmt.Sprintf("%s%s=%s", common.SparkExecutorEnvVarConfigKeyPrefix, key, value)
		envVarConfOptions = append(envVarConfOptions, envVar)
	}
	return envVarConfOptions
}
...
func GetDriverSecretConfOptions(app *v1beta2.SparkApplication) []string {
	...
	for _, s := range app.Spec.Driver.Secrets {
		conf := fmt.Sprintf("%s%s=%s", common.SparkKubernetesDriverSecretsPrefix, s.Name, s.Path)
		args = append(args, conf)
		if s.Type == v1beta2.SecretTypeGCPServiceAccount {
			conf = fmt.Sprintf(
				"%s%s=%s",
				common.SparkKubernetesDriverEnvPrefix,
				common.EnvGoogleApplicationCredentials,
				filepath.Join(s.Path, common.ServiceAccountJSONKeyFileName))
			args = append(args, conf)
			...
			conf = fmt.Sprintf(
				...
				filepath.Join(s.Path, common.HadoopDelegationTokenFileName))
			args = append(args, conf)
		}
	}
	return args
...
func GetExecutorSecretConfOptions(app *v1beta2.SparkApplication) []string {
	...
	for _, s := range app.Spec.Executor.Secrets {
		conf := fmt.Sprintf("%s%s=%s", common.SparkKubernetesExecutorSecretsPrefix, s.Name, s.Path)
		args = append(args, conf)
		if s.Type == v1beta2.SecretTypeGCPServiceAccount {
			conf = fmt.Sprintf(
				"%s%s=%s",
				common.SparkExecutorEnvVarConfigKeyPrefix,
				common.EnvGoogleApplicationCredentials,
				filepath.Join(s.Path, common.ServiceAccountJSONKeyFileName))
			args = append(args, conf)
		} else if s.Type == v1beta2.SecretTypeHadoopDelegationToken {
...
// PrintStatus return the status of a SparkApplication.
