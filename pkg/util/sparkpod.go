...
// GetAppName returns the application name by checking out pod labels.
func GetAppName(pod *corev1.Pod) (string, bool) {
	appName, ok := pod.Labels[common.LabelSparkAppName]
	return appName, ok
}

// GetSparkApplicationID returns the application ID by checking out pod labels.
