...
func (c *Controller) hasLastRunFinished(
	namespace string,
	lastRunName string) (bool, *v1beta1.SparkApplication, error) {
	...
	return app.Status.AppState.State == v1beta1.CompletedState ||
		app.Status.AppState.State == v1beta1.FailedState, app, nil
}
...
func (c *Controller) killLastRunIfNotFinished(namespace string, lastRunName string) error {
	...
	if err = c.crdClient.SparkoperatorV1beta1().SparkApplications(namespace).Delete(lastRunName,
...
func (c *Controller) checkAndUpdatePastRuns(
	...
	var toDelete []string
	if lastRunApp.Status.AppState.State == v1beta1.CompletedState {
		limit := 1
	...
	} else if lastRunApp.Status.AppState.State == v1beta1.FailedState {
		limit := 1
	...
	for _, name := range toDelete {
		c.crdClient.SparkoperatorV1beta1().SparkApplications(app.Namespace).Delete(name,
