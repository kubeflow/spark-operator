...
func NewProvider(client client.Client, name, namespace string, enableCertManager bool) *Provider {
...
func (cp *Provider) SyncSecret(ctx context.Context, name, namespace string) error {
	if cp.enableCertManager {
		return cp.SyncCertManagerSecret(ctx, name, namespace)
	}

	return cp.syncInternalCertSecret(ctx, name, namespace)
}

func (cp *Provider) syncInternalCertSecret(ctx context.Context, name string, namespace string) error {
	...
	if err := cp.parseInternalCertSecret(secret); err != nil {
		return err
	}
...
// SyncCertManagerSecret syncs the certificate secret generated by cert-manager.
func (cp *Provider) SyncCertManagerSecret(ctx context.Context, name string, namespace string) error {
	logger := ctrl.LoggerFrom(ctx, "name", name, "namespace", namespace)
	logger.Info("Syncing webhook secret")
	...
	if len(secret.Data[common.CACert]) == 0 ||
		len(secret.Data[common.TLSCert]) == 0 ||
		len(secret.Data[common.TLSKey]) == 0 {
		return fmt.Errorf("failed to sync cert manager certificate secret: %s, %s or %s is empty", common.CACert, common.TLSCert, common.TLSKey)
	}

	if err := cp.parseCertManagerSecret(secret); err != nil {
		return err
	}
...
// parseInternalCertSecret parses the internal certificate secret generated by the operator.
func (cp *Provider) parseInternalCertSecret(secret *corev1.Secret) error {
	if secret == nil {
	...
	if caCertPem == nil || caKeyPem == nil || serverCertPem == nil || serverKeyPem == nil {
...
// parseCertManagerSecret parses the certificate secret generated by cert-manager.
func (cp *Provider) parseCertManagerSecret(secret *corev1.Secret) error {
	if secret == nil {
		return fmt.Errorf("secret is nil")
	}

	caCertPem, _ := pem.Decode(secret.Data[common.CACert])
	serverCertPem, _ := pem.Decode(secret.Data[common.TLSCert])
	serverKeyPem, _ := pem.Decode(secret.Data[common.TLSKey])
	if caCertPem == nil || serverCertPem == nil || serverKeyPem == nil {
		return fmt.Errorf("failed to decode secret data to pem block")
	}
