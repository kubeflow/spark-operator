...
| controller.driverPodCreationGracePeriod | string | `"10s"` | Grace period after a successful spark-submit when driver pod not found errors will be retried. Also caps how long an in-flight spark-submit is given to exit after SIGTERM during controller shutdown (SIGTERM to the JVM, then up to this duration before SIGKILL). Must stay below controller-runtime GracefulShutdownTimeout (30s default) and below `terminationGracePeriodSeconds`. |
| controller.terminationGracePeriodSeconds | int | `60` | Termination grace period for the controller pod. The kubelet sends SIGKILL when this elapses. Must be set higher than controller-runtime GracefulShutdownTimeout (30s default) so the manager can interrupt and reap in-flight `spark-submit` subprocesses before the container is forcibly killed. |
