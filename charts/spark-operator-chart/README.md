...
| podDisruptionBudget | object | `{"enable":true,"minAvailable":3}` | podDisruptionBudget to avoid service degradation |
| podDisruptionBudget.enable | bool | `true` | Specifies whether to enable pod disruption budget. Ref: [Specifying a Disruption Budget for your Application](https://kubernetes.io/docs/tasks/run-application/configure-pdb/) |
| podDisruptionBudget.minAvailable | int | `3` | The number of pods that must be available. Require `replicaCount` to be greater than 1 |
...
| replicaCount | int | `3` | Desired number of pods, leaderElection will be enabled if this is greater than 1 |
