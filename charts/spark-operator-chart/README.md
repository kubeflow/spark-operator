...
| controller.gracefulShutdownTimeout | string | `"60s"` | Maximum time the controller manager waits for cancelled in-flight reconciles to return after SIGTERM before forcing the process to exit. The reconcile context is cancelled immediately on SIGTERM, so this window does not let reconciles run to completion; it gives them time to signal and reap the spark-submit JVM subprocess so a driver pod is not created moments before the controller dies. Set lower than `terminationGracePeriodSeconds` so the manager exits cleanly before the kubelet sends SIGKILL. |
| controller.terminationGracePeriodSeconds | int | `90` | Termination grace period for the controller pod. The kubelet sends SIGKILL when this elapses. Must be set higher than `controller.gracefulShutdownTimeout` so the manager has time to interrupt and reap in-flight `spark-submit` subprocesses before the process is forcibly killed. |
