...
| controller.defaultPodLabels | object | `{}` | Default labels to inject globally into every Spark driver and executor pod. These act as fallback defaults; labels defined directly in a SparkApplication spec will override these defaults if there are key conflicts. |
| controller.defaultPodAnnotations | object | `{}` | Default annotations to inject globally into every Spark driver and executor pod. These act as fallback defaults; annotations defined directly in a SparkApplication spec will override these defaults if there are key conflicts. |
