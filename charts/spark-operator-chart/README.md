...
| controller.defaultServiceAccount | string | `""` | Name of the service account used by the driver pod when a SparkApplication does not specify one. Leave empty to disable the fallback, in which case the driver pod uses the namespace's default service account. The service account must already exist in every namespace where Spark applications are submitted; note that `spark.serviceAccount.create` only provisions accounts for the namespaces listed in `spark.jobNamespaces`. |
